...
#             continue
        ...
        #########################
    for gene in ['Gag', 'Pol', 'Env']:
        file_aln = os.path.join(configs['path_out'], 'Gene_Cutter', f'{gene}.aa.fasta')
    
        if not os.path.exists(file_aln):
            logger.warning(f"{gene} not found in Gene Cutter output. Skipping.")
            continue  # <-- only skip this gene

        try:
            f_size = os.path.getsize(file_aln)
        except OSError:
            logger.warning(f"Could not access file size for {file_aln}. Skipping.")
            continue

        if f_size == 0:
            logger.warning(f"{file_aln} is empty. Skipping.")
            continue

    # proceed with further processing of `recs`
